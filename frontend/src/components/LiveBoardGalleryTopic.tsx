...
import { Image as ImageIcon, X, Upload, Loader2, RefreshCw, Zap, Maximize2 } from 'lucide-react';
...
import {
  useBoardGalleryTopic,
  useCreateBoardGalleryItemForTopic,
  useDeleteBoardGalleryItemTopic,
...
interface LiveBoardGalleryTopicProps {
  topicId: number;
  courseId: number;
}

export function LiveBoardGalleryTopic({ topicId, courseId }: LiveBoardGalleryTopicProps) {
  const { data: items = [], isLoading: loading, refetch, isRefetching: refreshing } = useBoardGalleryTopic(topicId);
  const createMutation = useCreateBoardGalleryItemForTopic(topicId);
  const deleteMutation = useDeleteBoardGalleryItemTopic(topicId);
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        createMutation.mutate({
          topic_id: topicId,
          title: autoTitle,
          description: `Quick upload from course ${courseId}`,
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      ...
                      <p className="text-white/70 text-xs">
                        {format(new Date(item.created_at), 'EEEE, d MMMM yyyy HH:mm', { locale: id })}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {format(new Date(selectedItem.created_at), 'EEEE, d MMMM yyyy HH:mm', { locale: id })}
                    </p>
                  </div>
                </div>
